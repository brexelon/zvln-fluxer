...
import {getBillingRepository, getGatewayService} from '../../middleware/ServiceRegistry';
import {
...
async function cancelStripeSubscriptionForDeletion(user: User, stripe: Stripe | null, userId: UserID): Promise<void> {
...
async function deleteUserAssociatedData(userId: UserID, deps: UserDeletionDependencies): Promise<void> {
	const {
		userRepository,
		guildRepository,
	...
	await Promise.all([
		...
		userRepository.deleteAllPrivateChannels(userId),
	]);
}

async function leaveAllGuilds(userId: UserID, deps: UserDeletionDependencies): Promise<void> {
	const {userRepository, guildRepository, storageService, purgeQueue, gatewayService} = deps;
...
async function leaveAllGroupDms(userId: UserID, deps: UserDeletionDependencies, createRecipientRemoveMessage: boolean): Promise<void> {
	...
	for (const channel of groupDmChannels) {
		try {
			...
			await userRepository.closeDmForUser(userId, channel.id);
			if (createRecipientRemoveMessage) {
...
export async function processUserDeletion(
	...
	await cancelStripeSubscriptionForDeletion(user, stripe, userId);
	...
	await leaveAllGuilds(userId, deps);
	await leaveAllGroupDms(userId, deps, true);
	...
	await deleteUserAssociatedData(userId, deps);
...
export async function purgeAllUserDataFromDatabase(userId: UserID, deps: UserDeletionDependencies): Promise<void> {
	const {userRepository, userCacheService, stripe, channelRepository, gatewayService, storageService, purgeQueue} =
		deps;
	Logger.debug({userId}, 'Starting full user data purge');
	const user = await userRepository.findUnique(userId);
	if (!user) {
		Logger.warn({userId}, 'User not found, skipping purge');
		return;
	}
	await cancelStripeSubscriptionForDeletion(user, stripe, userId);
	await leaveAllGuilds(userId, deps);
	await leaveAllGroupDms(userId, deps, false);
	Logger.debug({userId}, 'Deleting user messages');
	const messageDeletionService = new UserMessageDeletionService({
		channelRepository,
		gatewayService,
		storageService,
		purgeQueue,
	});
	await messageDeletionService.deleteUserMessagesBulk(userId);
	await deleteUserAssociatedData(userId, deps);
	await userRepository.deleteUser(userId);
	await userCacheService.invalidateUserCache(userId);
	Logger.debug({userId}, 'Full user data purge completed successfully');
}
